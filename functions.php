...
add_event_information($_REQUEST['date'],$_REQUEST['title'],$_REQUEST['user_id'],$_REQUEST['users']);
break;
default:
break;
}
...
<option value="0">Select</option>
                   ...
                   require_once 'process/dao.php';
                    ...
                    foreach ($arr as $value) {
                     ?>
                  <option value="<?php echo $value['id']; ?>"><?php echo $value['uuid']?></option>
                    <?php } ?>
...
<p><b>Event Title: </b><textarea class="form-control" id="eventTitle" name="eventTitle" placeholder="Write your Events"></textarea></p>
...
<div id="event_list" class="modal1"></div>
<!-- End of event list popup -->

<!--Below Code for Event Add-->

<!-- Popup div start here -->
<div id="event_add" class="modal1">
...
$result = $conn->query("SELECT title FROM events WHERE date = '".$currentDate."' ");
...
$.ajax({
type:'POST',
url:'fetch_user_details_by_id.php',
data:'q='+q,
success:function(html){
// alert(html);
$('#txtHint').html(html);
...
type:'POST',
url:'functions.php',
data:'fun_type=add_event_information&date='+date+'&title='+title+'&users='+users+'&user_id='+user_id,
...
include 'helpers/config.php';
$eventListHTML = '';
$date = $date?$date:date("d/m/yy H:i:s");
//Get events based on the current date
if($_SESSION["user_level"]==1){
$result = $conn->query("SELECT e.title,e.users,u.en_firstname,u.en_lastname FROM events e,users u WHERE date = '".$date."' and u.id=e.user_id");
...
$eventListHTML .= '<li> <b>User Name : </b>'.$row['en_firstname'].'&nbsp;'.$row['en_lastname'].'<br><b>Task : </b>'.$row['title'].'</li>';
}
$eventListHTML .= '</ul>';
$eventListHTML .= '</div>';
}
}else{
$result = $conn->query("SELECT e.title,e.users FROM events e,users u WHERE date = '".$date."' and u.id=e.user_id");

if($result->num_rows > 0){
$eventListHTML .= '<div class="modal-content1">';
$eventListHTML .= '<span class="close"><a href="#" onclick="close_popup("event_list")">×</a></span>';
$eventListHTML .= '<h2>Events on '.date("l, d M Y",strtotime($date)).'</h2>';
$eventListHTML .= '<ul>';
while($row = $result->fetch_assoc()){
$eventListHTML .= '<li> <b>Task : </b>'.$row['title'].'</li>';
}
$eventListHTML .= '</ul>';
$eventListHTML .= '</div>';
}
...
**********************************************************/
function add_event_information($date,$title,$user_id,$users){
...
$insert = $conn->query("INSERT INTO events (user_id,users,title,date,created,modified) VALUES ('".$user_id."','".$users."','".$title."','".$date."','".$currentDate."','".$currentDate."')");
if($insert){
echo 'ok';
}else{
echo 'err';
